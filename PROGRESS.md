...
1. Ejecutar: `npm run dev` (o `cargo tauri dev` desde src-tauri/)
2. Crear empresa en Config. Empresa (nombre, NIT, representante)
3. Crear firmante en Config. Firmante (nombre, CC, imagen de firma opcional)
4. Preparar Excel formato `NIT-nombre.xlsx` con 10 headers
5. Cargar Excel → Seleccionar Tercero → Revisar datos → Procesar → Seleccionar Firmante → Generar PDFs
6. Usar boton "Cargar Nuevo Excel" para iterar sin recargar la app

### Para build de produccion:

1. `npm run build:desktop` (o `npm run build:mac` / `npm run build:mac-arm` para solo macOS)
...
- **PDF**: Backend (Handlebars → HTML); frontend (jsPDF + html2canvas → PDF, write_pdf_file)
...
3. `src-tauri/src/services/crypto/pdf_generator.rs` — Pipeline completo:
   ...
   - `prepare_pdf_htmls()` — genera HTML por certificado (Handlebars); el frontend convierte a PDF con jsPDF/html2canvas
   - `write_pdf_file()` — escribe bytes PDF (base64) en disco
   - (Antes se usaba `generate_pdfs` con wkhtmltopdf; reemplazado por flujo frontend.)
4. Commands `prepare_pdf_htmls` y `write_pdf_file` en report_commands.rs — el frontend genera PDF y guarda con write_pdf_file
...
- PDF generado en frontend (jsPDF + html2canvas); backend solo prepara HTML (Handlebars)
...
### Generación de PDF (actual):

- **Backend (Rust):** Handlebars genera HTML por certificado; `prepare_pdf_htmls` devuelve HTML; `write_pdf_file` escribe bytes PDF (base64) en disco.
- **Frontend (Angular):** jsPDF + html2canvas convierten el HTML a PDF; el usuario elige carpeta y la app guarda con `write_pdf_file`. Sin binarios externos.
- Templates HTML/CSS con Handlebars para control de estilos. Archivos: `NIT_ID_AG{YEAR}.pdf`.
...
- (Posteriormente el flujo PDF pasó a frontend con jsPDF/html2canvas; ya no se usa html_to_pdf.)
- Logging mejorado con rutas completas de archivos
- (Generación PDF ahora en frontend; no se usa wkhtmltopdf.)
...
| PDF engine         | jsPDF + html2canvas (frontend), Handlebars (backend HTML) | Sin binarios externos; PDF generado en frontend |
...
| Tabla prefix       | crypto\_                                                  | Modularidad BD por herramienta                  |
